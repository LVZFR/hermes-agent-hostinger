...
- **Platform "configured" but bot never responds (most common)**: `hermes status` shows the platform as `✓ configured`, but messages get no reply. The usual cause is simply that **the gateway process isn't running** — config being valid and the gateway being up are two independent things. Diagnose in this order before touching any config:
  1. `hermes gateway status` — if it says "not running", that's the whole problem. Start it (see below).
  2. Validate the bot token against the platform API so you don't chase a second issue. For Telegram, grab `TELEGRAM_BOT_TOKEN` from the `.env` (path: `hermes config env-path` — it is often NOT under the project dir; e.g. `/opt/data/.env`, not `/opt/hermes/.env`) and `curl -s https://api.telegram.org/bot<TOKEN>/getMe`. `{"ok":true,...}` means the token is good and the problem is purely the gateway not running.
  3. Only after both above check out, look at allowed-users / home-channel config.
  Note: when validating tokens via the `terminal` tool, command substitution like `$(grep ...)` may be masked/blocked by secret-redaction — write a tiny script to a file and run it instead of inlining the token extraction.
- **Gateway running + Telegram connected, but `send_message list` / channel directory shows 0 targets**: This is a SEPARATE failure mode from "gateway not running". The gateway log shows a clean connect (`[Telegram] Connected to Telegram (polling mode)`, `✓ telegram connected`, `Gateway running with 1 platform(s)`) but the very next line reads `Channel directory built: 0 target(s)`. Zero targets with NO `rejected`/`unauthorized`/`not allowed` warnings in `logs/errors.log` almost always means **the bot has never received an inbound message, so it doesn't know any chat exists yet**. Telegram bots cannot initiate conversations — the *user* must open the bot in Telegram and press **Start** / send any message first. Until then the channel directory stays empty and there is nothing for `send_message` to target. Fix: tell the user to message the bot, then re-check `send_message list` and `tail logs/gateway.log` to confirm the chat registered. Config (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_ALLOWED_USERS`, `TELEGRAM_HOME_CHANNEL`) being valid does NOT populate the directory — only an inbound message does. If the user messages the bot and gets *no reply at all* (or an "unauthorized" reply), THEN it's an allowed-users mismatch: have them confirm their numeric Telegram user ID via @userinfobot and reconcile it with `TELEGRAM_ALLOWED_USERS` in the `.env`.
- **`hermes gateway install` is a no-op inside Docker**: in a container there's no systemd/user-service manager, so `install` prints a notice and does nothing ("Service installation is not needed inside a Docker container — the container runtime is your service manager"). Use `hermes gateway run` (background it, e.g. `terminal(background=true)`) for the current container lifetime, and for persistence across container restarts bake it into the container entrypoint or run the container with `--restart unless-stopped` plus a startup command that launches the gateway. Don't keep retrying `install`.
